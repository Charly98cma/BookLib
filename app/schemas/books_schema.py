...
_book_example : Dict[str, str | int | float | List[str]] = {
    ...
    "authors": [
        "52f19e39-20ac-47fd-9df2-53d58c0b3f64",
    ],
}
...
class BookCreate(BaseModel):
    ...
    hc_rating: Annotated[Optional[float], Field(examples=[_book_example["hc_rating"]])]
    ...
    authors: Annotated[List[uuid.UUID], Field(examples=[_book_example["authors"]])]
...
class BookDBResponse(BaseModel):
    ...
    def extract_author_ids(cls, author_list: List[Author]) -> List[uuid.UUID]:
        return [author.id for author in author_list]
